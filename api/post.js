...
async function renderCard(post) {
  ...
  const { error: uploadErr } = await db.storage
    .from('anvil-cards').upload(fileName, buffer, { contentType: 'image/png', upsert: true });
...
function buildCardHtml(post) {
  const { quote, bg_color, border_color, ink_color, font_quote, font_header, font_meta,
          font_size_quote, font_size_header, font_size_meta, post_number, scheduled_at, dimension } = post;
  ...
  const muted = blendHex(ink_color || '#e8e4dc', bg_color || '#2e4a6a', 0.45);
